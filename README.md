...
| Команда | Описание |
|---------|----------|
| `npm run dev` | Сборка и запуск с `.env` |
| `npm run build` | Компиляция TypeScript |
| `npm run start` | Запуск скомпилированного приложения |
| `npm run clean` | Очистка папки `dist` |
| `npm run cb` | Очистка и сборка |
| `npm run cbr` | Очистка, сборка и запуск |

### Windows

| Команда | Описание |
|---------|----------|
| `npm run wcb` | Очистка и сборка |
| `npm run wcbr` | Очистка, сборка и запуск |
...
Тесты запускаются через [Vitest](https://vitest.dev). Перед запуском тестов проект автоматически пересобирается.

## Структура проекта

```
src/
├── clients/        # HTTP-клиенты (Telegram, Currency API)
...
| `/currency` | Инструкция по получению курса валют |

Для получения курса валют отправь пару в формате `USD/RUB`.
...
| № | Команда | Описание |
|---|---------|----------|
| 1 | \x1b[32m | включает зелёный цвет |
| 2 | \x1b[0m | сбрасывает обратно |
